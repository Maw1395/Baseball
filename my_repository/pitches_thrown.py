...
for i in Pitcher:
	if(startingPitcher!=i[1]):
		...
		if inningPlusOuts not in INNINGS:
			...
			k=inningPlusOuts
			RUNS[inningPlusOuts]=(pitches/(math.floor(k)*3+math.floor(((k-math.floor(k))*10))))
			if balls==0:
				balls=1
		else:
			INNINGS[inningPlusOuts]+=1	
...
plt.xlabel("Inning")
plt.ylabel("Average Pitches Thrown vs Number of outs")
...
plt.title("2000-2009 Pitchers Pulled By Inning vs Average Pitches Thrown vs Number of outs")
font = {'family' : 'normal',
        'weight' : 'bold',
        'size'   : 22}
plt.rc('font', **font)
