...
INNINGS={}
stmt = "select Game_Number, Pitcher, outs, inning, outcome, runs from PlateAppearance group by game_number, pitcher, batter_number;"
...
for i in Pitcher:
	...
	if "Flyball" in i[4] or "out" in i[4]:
		...
		thisout=i[2]
	inningPlusOuts = str(i[3])+"."+str(thisout)
	thisruns=i[5]
print INNINGS
print RUNS
outlist=[]
...
plt.xlabel("Inning")
plt.ylabel("Average Number of Runs Given Up")
...
plt.title("2000-2009 Inning and Outs vs Average Runs Given Up")
font = {'family' : 'normal',
        'weight' : 'bold',
        'size'   : 22}
plt.rc('font', **font)
