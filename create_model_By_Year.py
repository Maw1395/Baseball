...
def main(year):
	...
	test_y = []
	train_y = []
	with open(str(year)+'-Data1.csv', 'rb') as csvfile:
	...
	num = 0
	with open(str(year)+'-Labels1.csv', 'rb') as csvfile:
		data = csv.reader(csvfile)
		for row in data:
			if num == 0:
				num = num + 1
	...
	cnt=Counter()
	#print len(x_test)
	rf = load('RFE-Smote.joblib')
	#rf.fit(features,labels)
	hold = rf.predict(x_test)
	...
	'''
	with open(str(year)+"-WoodyG.csv", "wb") as csv_file:
		writer = csv.writer(csv_file)
		for line in hold:
			writer.writerow(line)
	with open(str(year)+"-WoodyT.csv", "wb") as csv_file:
		writer = csv.writer(csv_file)
		for line in y_test:
			writer.writerow(line)
	...
	'''		
	with open(str(year)+"-Info.csv", "wb") as csv_file:
		#writer = csv.writer(csv_file)
	...
	plt.savefig(str(year)+"-Importance") 
	'''
